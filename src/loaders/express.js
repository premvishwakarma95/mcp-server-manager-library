...
module.exports = function expressLoader(app) {
  ...
  // The basicAuth gate runs first: when ADMIN_UI_USER + ADMIN_UI_PASSWORD are
  // configured, the browser pops up a native login prompt before the HTML is
  // served. Empty creds = gate off (dev convenience).
  const adminUiDir = path.join(__dirname, '..', 'public', 'admin');
  app.use(
    '/admin',
    basicAuth({ user: env.admin.uiUser, pass: env.admin.uiPass }),
    express.static(adminUiDir, { index: 'index.html' })
  );
