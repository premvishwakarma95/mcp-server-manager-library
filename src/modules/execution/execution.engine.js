...
// Path-template placeholders like `/tasks/{task_gid}` consume their key from
// the validated input. If we leave that key in the body too, strict APIs
// reject it (Asana: "Unrecognized request field `task_gid`"). Return the set
// of keys the endpoint template references so the executor can strip them.
function extractPathTemplateKeys(endpoint) {
  if (!endpoint || typeof endpoint !== 'string') return [];
  const keys = [];
  endpoint.replace(/\{([a-zA-Z0-9_]+)\}/g, (_m, key) => { keys.push(key); return _m; });
  return keys;
}
...
function safeTruncateForLog(body) {
  ...
  try {
    ...
    // Wrap in a structured envelope so the stored body is always valid JSON
    // (not a broken half-string), and the inspector can tell at a glance how
    // much was clipped. `preview` is plain text — never parsed.
    return {
      value: {
        _truncated: true,
        _originalBytes: bytes,
        _maxBytes: maxBytes,
        _hint: 'Body exceeded EXECUTION_LOG_BODY_BYTES. Raise it in .env to keep more.',
        preview: str.slice(0, maxBytes),
      },
      truncated: true,
    };
  } catch (_) {
    return { value: { _truncated: true, _error: 'unserializable' }, truncated: true };
  }
}
...
async function executeTool({ server, tool, input = {}, requestId = null, headersOverride = {}, callerHeaders = {} }) {
  ...
  // Keys that were substituted into the URL path. Send them in the URL only —
  // not in the body (Asana, JSON:API, etc. reject unknown top-level body keys)
  // and not as redundant query params on GET/DELETE.
  const pathKeys = extractPathTemplateKeys(tool.endpoint || '');
  const inputWithoutPathKeys = (() => {
    if (!validated || typeof validated !== 'object' || Array.isArray(validated)) return validated;
    if (pathKeys.length === 0) return validated;
    const out = { ...validated };
    for (const k of pathKeys) delete out[k];
    return out;
  })();
  ...
  if (method === 'GET' || method === 'DELETE') {
    paramsForRequest = { ...(tool.queryParams || {}), ...(inputWithoutPathKeys || {}) };
  } else {
    bodyForRequest = inputWithoutPathKeys;
